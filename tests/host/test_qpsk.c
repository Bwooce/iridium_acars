...
static int test_direction(const int *uw, const char *name, ir_direction_t expected)
{
    ...
    if (!rc) {
        printf("  %s: FAIL — demod returned 0\n", name);
        return 0;
    }
...
int main(void)
{
    ...
    if (test_direction(UW_DL, "DL", DIR_DOWNLINK))
        passed++;
    else
        failed++;

    printf("Test 2: uplink UW recognition\n");
    if (test_direction(UW_UL, "UL", DIR_UPLINK))
        passed++;
    else
        failed++;

    printf("Test 3: unknown UW returns 0\n");
    if (test_unknown_direction())
        passed++;
    else
        failed++;
