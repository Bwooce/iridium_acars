...
static void apply_drift_inplace(int16_t *iq, int n_complex, double f_shift_hz, double fs_hz)
{
    ...
    for (int i = 0; i < n_complex; i++) {
        ...
        if (yi > 32767.0) yi = 32767.0; else if (yi < -32768.0) yi = -32768.0;
        if (yq > 32767.0) yq = 32767.0; else if (yq < -32768.0) yq = -32768.0;
        iq[i*2 + 0] = (int16_t)yi;
        iq[i*2 + 1] = (int16_t)yq;
...
static int run_pipeline(int16_t *iq25, int n25, int *out_bch)
{
    ...
    if (!window_25 || !window_250 || !scr_in_i || !scr_in_q || !scr_out_i || !scr_out_q) {
        free(window_25); free(window_250); free(scr_in_i); free(scr_in_q);
        free(scr_out_i); free(scr_out_q); return -1;
    }
