                                ...
                                const uint8_t *bits, int n_bits)
{
    ...
    for (int g = 0; g < FIXTURE_ALBQ_RAW_GOLDEN_COUNT; g++) {
        ...
        int64_t score = (ds * 100 / HG_TIME_TOL_2500K) + (df * 100 / HG_FREQ_TOL_HZ);
        if (score < best_score) {
            best_score = score;
            best       = g;
        }
    ...
    if (n_bits == e->gri_n_bits)
        hg_len_eq++;
    else if (n_bits < e->gri_n_bits)
        hg_len_short++;
...
// Multi-frame emission callback context. Each burst tag's processing
// feeds one of these into burst_pipeline_process_burst; the callback
// fires once per decoded sub-frame.
typedef struct {
    int      n_emitted;
    uint64_t begin;
    float    rel_freq_hz;
    ...
    int      center_bin;
    bool     dump_first_uw;
    bool    *p_dump_first_uw_fired;
    int     *p_uw_found;
} wb_cb_ctx_t;

void wb_cb_emit(burst_pipeline_result_t *r, void *c)
{
    wb_cb_ctx_t *ctx = (wb_cb_ctx_t *)c;
    if (r->uw_res.direction != UW_DIR_UNKNOWN) (*ctx->p_uw_found)++;
    if (!r->demod_ok) {
        free(r->frame.bits);
        return;
    }
    if (ctx->dump_first_uw && !*ctx->p_dump_first_uw_fired) {
        printf("FIRST-UW-LOCK burst tag #%d bin=%d -- set DUMP_TARGET_BIN=%d "
               "and re-run to dump this burst's stages.\n",
               ctx->tag_idx, ctx->center_bin, ctx->center_bin);
        *ctx->p_dump_first_uw_fired = true;
    }
    host_golden_compare((uint32_t)ctx->begin, ctx->rel_freq_hz,
                        r->frame.bits, r->frame.n_bits);
    free(r->frame.bits);
    ctx->n_emitted++;
}
...
int main(void)
{
    ...
    for (int i = 0; i < n_tags; i++) {
        ...
        // Multi-frame: gri's burst_downmix emits one frame per
        // process_next_frame iteration. Use the callback API so we
        // catch ALL sub-frames within multi-frame bursts (gri_id=N+1,
        // N+2, ...). Without this we only see the FIRST sub-frame and
        // miss the rest -- 7+ missed bursts in the ALBQ corpus.
        int         signed_bin  = tags[i].center_bin - FBT_FFT_SIZE / 2;
        float       rel_freq_hz = (float)signed_bin * (float)INPUT_FS_HZ / (float)FBT_FFT_SIZE;
        wb_cb_ctx_t cbctx       = {
                  .n_emitted             = 0,
                  .begin                 = (uint64_t)begin,
                  .rel_freq_hz           = rel_freq_hz,
                  .tag_idx               = i,
                  .center_bin            = tags[i].center_bin,
                  .dump_first_uw         = dump_first_uw,
                  .p_dump_first_uw_fired = &dump_first_uw_fired,
                  .p_uw_found            = &uw_found,
        };
        int n_frames = burst_pipeline_process_burst(
            window_250, n_out, wb_cb_emit, &cbctx);
        if (n_frames > 0) pipeline_ok++;
        decoded += cbctx.n_emitted;
    }
