...
static void test_two_tone(int bin_a, int bin_b) {
    ...
    for (int k = 0; k < N; k++) {
        ...
        if (mag2 > m1) { m2 = m1; top2 = top1; m1 = mag2; top1 = k; }
        else if (mag2 > m2) { m2 = mag2; top2 = k; }
    }
    int matched = (top1 == bin_a && top2 == bin_b)
                || (top1 == bin_b && top2 == bin_a);
    char detail[128];
...
static void test_random_vs_dft(void) {
    ...
    for (int k = 0; k < N; k++) {
        ...
        if (e > max_err) { max_err = e; max_err_k = k; }
    }
