...
static int parse_manifest_first(const char *path, manifest_t *m) {
    ...
    if (!fgets(line, sizeof(line), fh)) { fclose(fh); return -1; }   // first row
...
static int load_cf32(const char *path, float **out, int *n_complex) {
    ...
    fclose(fh);
    if ((int)got != 2 * n) { free(buf); return -1; }
    *out = buf;
    *n_complex = n;
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (!iq_pre) { free(raw); free(ref); return 2; }
    ...
    if (!iq_post) { free(iq_pre); free(raw); free(ref); return 2; }
