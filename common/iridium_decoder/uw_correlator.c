    ...
    for (int n = 0; n < ntaps; n++) {
        ...
        if (abs_k < 1e-6f) {
            ...
            if (fabsf(denom) < 1e-9f) {
                ...
                float num = sinf(PI * (1.0f - beta) * k) + four_bk * cosf(PI * (1.0f + beta) * k);
                v         = num / denom;
            }
        }
        taps[n] = v;
...
static void cfo_init(void)
{
    ...
    for (int i = 0; i < CFO_INPUT_N; i++) {
        ...
        float w              = 0.42f - 0.5f * cosf(2.0f * PI * t) + 0.08f * cosf(4.0f * PI * t);
                               ...
                               int uw_offset_complex)
{
...
#ifdef UW_CORRELATOR_CFO_DEBUG
    {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            float omega_dbg = -2.0f * 3.14159265358979323846f * kf_dbg / (float)N * ((float)UW_SPS / 2.0f);
                        ...
                        uw_corr_result_t *out_result)
{
    ...
    for (int k = 0; k < search_complex; k++) {
        ...
        if (m2 > best_dl) {
            best_dl    = m2;
            best_dl_k  = k;
            best_dl_re = ifft_re[idx];
            best_dl_im = ifft_im[idx];
        }
    }
    for (int k = 0; k < CORR_FFT_N; k++) {
        int64_t ar = burst_re[k], ai = burst_im[k];
    ...
    for (int k = 0; k < search_complex; k++) {
        ...
        if (m2 > best_ul) {
            best_ul    = m2;
            best_ul_k  = k;
            best_ul_re = ifft_re[idx];
            best_ul_im = ifft_im[idx];
        }
    }
#endif
                                   ...
                                   int n_complex, int search_max)
{
    ...
    int shift = 0;
    while ((max_mag2 >> shift) > INT16_MAX)
        shift++;
                             ...
                             int n_complex)
{
    ...
    if (fir_len > s_rrc_scr_cap) {
        ...
        if (!s_rrc_scr_in_i || !s_rrc_scr_in_q || !s_rrc_scr_out_i || !s_rrc_scr_out_q) {
