...
static void aggregator_ingest_task(void *arg)
{
    ...
    while (1) {
        ...
        // No soft metrics: the PDU wire format carries hard bits only,
        // so the Chase-2 fallback is inert on the aggregator path.
        frame_decoder_push(s_bits01, pdu.n_bits, NULL, 0, dir, 0u,
