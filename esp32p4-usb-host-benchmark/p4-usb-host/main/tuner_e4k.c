...
static int __attribute__((unused)) is_fvco_valid(uint32_t fvco_z)
{
    ...
    {
        fprintf(stderr, "[E4K] Fvco %lu invalid\n", fvco_z);
        return 0;
    }
...
static int is_fosc_valid(uint32_t fosc)
{
    ...
    {
        fprintf(stderr, "[E4K] Fosc %lu invalid\n", fosc);
        return 0;
    }
...
static int __attribute__((unused)) is_z_valid(uint32_t z)
{
    if (z > 255)
    {
        fprintf(stderr, "[E4K] Z %lu invalid\n", z);
        return 0;
    }
...
int e4k_tune_freq(struct e4k_state *e4k, uint32_t freq)
{
    ...
    if (!(rc & 0x01))
    {
        fprintf(stderr, "[E4K] PLL not locked for %lu Hz!\n", freq);
        return -1;
    }
...
int e4k_dc_offset_gen_table(struct e4k_state *e4k)
{
    ...
    for (i = 0; i < ARRAY_SIZE(dc_gain_comb); i++)
    {
        ...
        fprintf(stderr, "[E4K] Table %lu I=%u/%u, Q=%u/%u\n",
